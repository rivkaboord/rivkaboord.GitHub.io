...
# Load Data
path_train            <- "Programming and Data Science/R/HR_201_Employee_Attrition_Project/00_Data/telco_train.xlsx"
path_test             <- "Programming and Data Science/R/HR_201_Employee_Attrition_Project/00_Data/telco_test.xlsx"
path_data_definitions <- "Programming and Data Science/R/HR_201_Employee_Attrition_Project/00_Data/telco_data_definitions.xlsx"
...
# Processing Pipeline
source("Programming and Data Science/R/HR_201_Employee_Attrition_Project/00_Scripts/data_processing_pipeline.R")
...
automl_leader <- h2o.loadModel("Programming and Data Science/R/HR_201_Employee_Attrition_Project/04_Modeling/h2o_models/StackedEnsemble_BestOfFamily_AutoML_20210801_144306")
...
glimpse(rates_by_threshold_tbl)
...
source("..//Documents/Programming and Data Science/R/HR_201_Employee_Attrition_Project/00_Scripts/assess_attrition.R")
...
calculate_savings_by_threshold <- function(data, h2o_model, threshold = 0,
    ...
    pred_0_tbl <- h2o_model %>%
        predict(newdata = as.h2o(data_0_tbl)) %>%
        as_tibble() %>%
        bind_cols(
            data_0_tbl %>%
                select(EmployeeNumber, MonthlyIncome, OverTime)
...
test_tbl %>% calculate_savings_by_threshold(automl_leader, threshold = 0,
                                            tnr = 0, fnr = 0, tpr = 1, fpr = 1)

# Do Nothing Policy

test_tbl %>% calculate_savings_by_threshold(automl_leader, threshold = 1,
...
rates_by_threshold_optimized_tbl %>%
    ...
    geom_label(aes(label = scales::dollar(savings %>% round(0))),
               vjust = -1, color = palette_light()[[2]],
               data = rates_by_threshold_optimized_tbl %>% filter(threshold == max(threshold))) +
    ...
    labs(title = "Optimization Results: Expected Savings Maximized At 12.8%",
         x = "Threshold (%)",
         y = "Savings")
...
calculate_savings_by_threshold_2 <- function(data, h2o_model, threshold = 0,
    ...
    # 4.1 Calculate Expected Value with OT
    ...
    # 4.2 Calculate Expected Value with Targeted OT
...
test_tbl %>%
    calculate_savings_by_threshold_2(automl_leader,
                                     avg_overtime_pct = 0.10,
                                     net_revenue_per_employee = 2500000)

# 6.2 Sensitivity Analysis ----

max_savings_rates_tbl <- rates_by_threshold_optimized_tbl %>% filter(savings == max(savings))

calculate_savings_by_threshold_2(test_tbl, automl_leader,
